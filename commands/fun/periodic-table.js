...
module.exports = class PeriodicTableCommand extends Command {
	constructor(client) {
		super(client, {
			...
			args: [
				{
					key: 'element',
					prompt: 'I need element you want to find. You can enter the name, symbol, or atomic number.',
					type: 'string',
					validate: element => {
	...
	async run(msg, { element }) {
		...
		msg.say(`**${element.name} (${element.symbol})** is a ${element.phase} in ${period}.`,{ files: [{ attachment: canvas.toBuffer(), name: `${element.name}.png` }] });
